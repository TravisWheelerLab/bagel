 ...
 * 
 * 
 * The <.out file> from an transmark benchmark consists of lines:
 ...
 * appending ".tbl" and ".pos" to the <transmark> argument;
 * that is:
 *   : <transmark>.tbl contains one line per query: 1st field is name of query
 *   : <transmark>.pos contains one line per positive: 1st field is the
 ...
 *
 * A typical command line, after having run a benchmark on "transmark" under
 * MPI with many output files:
 * 
 *    cat *.out | sort -g | ./transmark-rocplot transmark - > results.xy
...
static char banner[] = "construct a ROC plot of transmark results, using Bayesian bootstrapping";
static char usage[]  = "[options] <transmark_basename> <.out file>\n";
...
static int    parse_results_transmark(char *resfile, int **pni, ESL_KEYHASH *modelkh, ESL_KEYHASH *poskh, struct result_s **ret_r, int *ret_nr, int *ret_nneg);
...
main(int argc, char **argv)
{
  ...
  parse_results_transmark(resfile, pni, qkh, poskh, &rp, &nresults, &nneg);
  nseq = npos+nneg;
 ...
 */
static int
parse_results_transmark(char *resfile, int **pni, ESL_KEYHASH *qkh, ESL_KEYHASH *poskh, struct result_s **ret_r, int *ret_nr, int *ret_nneg)
{
  ESL_FILEPARSER  *efp    = NULL;
  char            *tok    = NULL;
  char            *match  = NULL;
